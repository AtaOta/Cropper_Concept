...
function contentMousedown(e){
    ...
    function mousemove(e){
        if(!IsResizing){
            ...
            rectMaxTop = ((imgWrapperRect.top + heightChange/2 + imgMoveY) <= (wrapperTop)) ? (wrapperTop):(imgWrapperRect.top + heightChange/2 + imgMoveY);
            rectMaxBottom = ((imgWrapperRect.bottom - heightChange/2 + imgMoveY) >= (wrapperBottom)) ? (wrapperBottom):(imgWrapperRect.bottom - heightChange/2 + imgMoveY);
...
const DetetctRotate = () =>{
    ...
    ImageWrapper.style.transform = `translate(${imgMoveX}px, ${imgMoveY}px) rotate(${rotate}deg)`;
}
